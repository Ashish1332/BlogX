...
import { insertBlogSchema, insertCommentSchema, insertMessageSchema } from "./shared/schema.js";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/blogs/trending", async (req, res) => {
    try {
      ...
      if (req.isAuthenticated()) {
        ...
        const currentUserId = req.user?._id?.toString();
        if (!currentUserId || currentUserId.length !== 24) {
          console.error("Invalid user ID for trending blogs:", currentUserId);
  ...
  app.delete("/api/comments/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!comment) {        console.log(`No comment found with ID: ${commentId}`);
