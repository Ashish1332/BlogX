...
async function seed() {
  try {
    ...
    const blogs = await db.insert(schema.blogs).values(blogsData).returning();
    ...
    const followersData = [
      ...
      { followerId: users[5].id, followingId: users[2].id },
      { followerId: users[0].id, followingId: users[3].id },
      { followerId: users[2].id, followingId: users[3].id },
      { followerId: users[5].id, followingId: users[3].id },
      { followerId: users[0].id, followingId: users[4].id },
      { followerId: users[1].id, followingId: users[5].id },
      { followerId: users[2].id, followingId: users[5].id },
      { followerId: users[3].id, followingId: users[5].id }
    ];

    await db.insert(schema.followers).values(followersData);
    console.log(`Created follower relationships`);

    // Create likes
    const likesData = [
      { userId: users[3].id, blogId: blogs[0].id },
      { userId: users[4].id, blogId: blogs[0].id },
      { userId: users[5].id, blogId: blogs[0].id },
      { userId: users[1].id, blogId: blogs[0].id },
      { userId: users[0].id, blogId: blogs[1].id },
      { userId: users[3].id, blogId: blogs[1].id },
      { userId: users[4].id, blogId: blogs[1].id },
      { userId: users[5].id, blogId: blogs[1].id },
      { userId: users[0].id, blogId: blogs[2].id },
      { userId: users[1].id, blogId: blogs[2].id },
      { userId: users[3].id, blogId: blogs[2].id },
      { userId: users[4].id, blogId: blogs[2].id },
      { userId: users[0].id, blogId: blogs[3].id },
      { userId: users[1].id, blogId: blogs[3].id },
      { userId: users[2].id, blogId: blogs[3].id },
      { userId: users[5].id, blogId: blogs[3].id },
      { userId: users[0].id, blogId: blogs[4].id },
      { userId: users[1].id, blogId: blogs[4].id },
      { userId: users[2].id, blogId: blogs[4].id },
      { userId: users[3].id, blogId: blogs[4].id },
      { userId: users[0].id, blogId: blogs[5].id },
      { userId: users[1].id, blogId: blogs[5].id },
      { userId: users[3].id, blogId: blogs[5].id },
      { userId: users[4].id, blogId: blogs[5].id }
    ];

    await db.insert(schema.likes).values(likesData);
    console.log(`Created likes`);

    // Create comments
    const commentsData = [
      { content: "Great insights! I've noticed the same trends in my company. The flexibility of async work has been a game-changer for our team.", userId: users[3].id, blogId: blogs[0].id },
      { content: "I'm curious how companies are handling the time zone challenges with remote teams. Any tips?", userId: users[1].id, blogId: blogs[0].id },
      { content: "This resonates with me so much. I'm using Copilot daily, and while it's helpful, it's definitely not replacing my job anytime soon!", userId: users[0].id, blogId: blogs[1].id },
      { content: "I'd add that understanding legacy code is another area where AI struggles. Context and history matter so much in real-world codebases.", userId: users[4].id, blogId: blogs[1].id },
      { content: "Just ordered 'Zero to One' based on your recommendation. I've been stuck in incremental thinking lately and need a fresh perspective.", userId: users[0].id, blogId: blogs[2].id },
      { content: "I'd add 'Range' by David Epstein to this list. It changed how I think about expertise and skill development.", userId: users[1].id, blogId: blogs[2].id },
      { content: "The point about reducing friction rather than adding features is so important. I've seen many products fail because they became too complex.", userId: users[2].id, blogId: blogs[3].id },
      { content: "How do you balance getting quick feedback with ensuring you're not biasing users with under-developed prototypes?", userId: users[5].id, blogId: blogs[3].id },
      { content: "The point about targeting the right investors is key. I wasted months pitching to VCs who never invest in hardware startups.", userId: users[1].id, blogId: blogs[4].id },
      { content: "What's your take on convertible notes vs. priced rounds for first-time fundraising?", userId: users[3].id, blogId: blogs[4].id },
      { content: "I've been making the mistake of creating content about our product instead of addressing customer problems. This was a wake-up call!", userId: users[0].id, blogId: blogs[5].id },
      { content: "Any tips for finding out where your audience actually spends time online? Surveys haven't been very reliable for us.", userId: users[3].id, blogId: blogs[5].id }
    ];
    ...
    const bookmarksData = [
      ...
      { userId: users[2].id, blogId: blogs[3].id },
      { userId: users[4].id, blogId: blogs[1].id },
      { userId: users[5].id, blogId: blogs[2].id }
    ];
